...
int commit_create(const char *message, ObjectID *commit_id_out) {
    // Step 1: Build a tree from the current index (staged state)
    ObjectID tree_id;
    ...
    // Step 3: Try to read the current HEAD as the parent
    ObjectID parent_id;
    ...
    // Step 6: Update HEAD (move branch pointer to new commit)
